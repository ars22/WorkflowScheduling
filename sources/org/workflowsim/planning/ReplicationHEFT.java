 ...
 * @date Oct 12, 2013
 */
public class ReplicationHEFT extends BasePlanningAlgorithm {
            ...
            for (Task parent : currentTask.getParentList()){
            	...
            	if (flag){
            		for (int i=0; i<repCount; i++){
            			Task newTask = parent;
            			newTask.cloudletId = (int)Math.pow(10, i) + parent.getCloudletId();
            			List <Task> updatedList = getTaskList();
            			updatedList.add(newTask);
            			Parameters.wfPlanner.setTaskList(updatedList);
